...
- **Video Knowledge System V2**: Professor OS "watches" every video using Gemini 2.5 Pro to process YouTube videos natively via direct URL analysis and extract deeply structured BJJ knowledge for intelligent coaching. Includes visual analysis of technique demonstrations, quality scoring (instruction/visual/audio), coaching notes, and automatic fallback to metadata-only analysis for private/unavailable videos. Analysis version tracking ('2.5' for Pro visual, '1.0' for legacy Flash) distinguishes old vs new analyses.
