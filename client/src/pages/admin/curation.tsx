...
import { 
  ...
  Loader2, Youtube, Plus, Activity, ArrowLeft
} from "lucide-react";
...
export default function CurationCommandCenter() {
  ...
  const { data: apiQuota } = useQuery<{ youtubeUsed: number; youtubeLimit: number; geminiCostToday: number; geminiCostMonth: number; }>({
    ...
    queryFn: () => adminApiRequest("/api/admin/curation/api-quota"),
  ...
  const toggleCuration = useMutation({
    mutationFn: async (enabled: boolean) => {
      return adminApiRequest("/api/admin/curation/toggle", "POST", { enabled });
    },
  ...
  const runCuration = useMutation({
    mutationFn: async (params: { type: string; query?: string; filter?: string }) => {
      return adminApiRequest("/api/admin/curation/run", "POST", params);
    },
    onSuccess: () => {
  ...
  const cancelJob = useMutation({
    mutationFn: async (jobId: string) => {
      return adminApiRequest(`/api/admin/curation/cancel/${jobId}`, "POST");
    },
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/admin/dashboard">
              <button className="p-2 hover:bg-zinc-800 rounded-lg" data-testid="button-back-dashboard">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={() => toggleCuration.mutate(!curationStatus?.isActive)} 
                  disabled={toggleCuration.isPending}
                  ...
                  data-testid="button-toggle-curation"
                >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-zinc-500" /><span>4x daily (3:15am, 9am, 3pm, 9pm EST)</span></div>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <div className="font-medium">{curationStatus?.nextRun || (curationStatus?.isActive ? 'Scheduled' : 'Paused')}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              <button 
                onClick={() => runCuration.mutate({ type: "meta" })} 
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              <input 
                ...
                className="w-full bg-zinc-700 border border-zinc-600 rounded px-3 py-2 placeholder-zinc-500"
                data-testid="input-custom-instructor"
              />
              <button 
                onClick={() => { runCuration.mutate({ type: "instructor", query: customInstructor || selectedInstructor }); setCustomInstructor(""); setSelectedInstructor(""); }}
                disabled={runCuration.isPending || (!selectedInstructor && !customInstructor)}
                className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2 px-4 rounded font-medium disabled:opacity-50"
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              />
              <button 
                onClick={() => { runCuration.mutate({ type: "technique", query: techniqueSearch }); setTechniqueSearch(""); }}
                disabled={runCuration.isPending || !techniqueSearch}
                className="w-full bg-green-600 hover:bg-green-500 text-white py-2 px-4 rounded font-medium disabled:opacity-50"
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              <button 
                onClick={() => { runCuration.mutate({ type: "position", query: selectedPosition }); setSelectedPosition(""); }}
                disabled={runCuration.isPending || !selectedPosition}
                className="w-full bg-yellow-600 hover:bg-yellow-500 text-white py-2 px-4 rounded font-medium disabled:opacity-50"
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              <button 
                onClick={() => { runCuration.mutate({ type: "gi-nogi", filter: selectedGiNogi }); setSelectedGiNogi(""); }}
                disabled={runCuration.isPending || !selectedGiNogi}
                className="w-full bg-red-600 hover:bg-red-500 text-white py-2 px-4 rounded font-medium disabled:opacity-50"
            ...
            <div className="bg-zinc-800 rounded-lg p-4 space-y-3">
              ...
              />
              <button 
                onClick={() => { runCuration.mutate({ type: "custom", query: customSearch }); setCustomSearch(""); }}
                disabled={runCuration.isPending || !customSearch}
                className="w-full bg-cyan-600 hover:bg-cyan-500 text-white py-2 px-4 rounded font-medium disabled:opacity-50"
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
          ...
          <div className="mb-6">
            ...
            {activeJobs && activeJobs.length > 0 ? (
              <div className="space-y-2">
                {activeJobs.map((job) => (
                  <div key={job.id} className="bg-zinc-800 rounded-lg p-3 flex items-center justify-between" data-testid={`job-active-${job.id}`}>
                    <div className="flex items-center gap-3">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(jobHistory || []).slice(0, 10).map((job) => (
                    <tr key={job.id} className="border-b border-zinc-800/50" data-testid={`job-history-${job.id}`}>
                      ...
                      <td className="py-2 px-3 text-center">
                        ...
                        {job.status === 'cancelled' && <AlertCircle className="w-4 h-4 text-yellow-400 inline" />}
                      </td>
                    </tr>
                  ))}
                  {(!jobHistory || jobHistory.length === 0) && (
                    <tr>
                      <td colSpan={6} className="py-4 text-center text-zinc-500">No job history</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
            ...
            <div className="space-y-2">
              {["BJJ Fanatics", "Bernardo Faria BJJ", "Lachlan Giles", "John Danaher", "B-Team Jiu Jitsu", "FloGrappling"].map((c) => (
                <div key={c} className="flex items-center justify-between py-2 px-3 bg-zinc-800 rounded" data-testid={`channel-${c.replace(/\s+/g, '-')}`}>
                  <span>{c}</span>
                </div>
              ))}
              <button className="flex items-center gap-2 text-sm text-lime-400 hover:text-lime-300 mt-2" data-testid="button-add-channel">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-mono">{apiQuota?.youtubeUsed?.toLocaleString() || 0} / {apiQuota?.youtubeLimit?.toLocaleString() || 10000} daily</span>
              </div>
              <div className="h-3 bg-zinc-800 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full ${(apiQuota?.youtubeUsed || 0) / (apiQuota?.youtubeLimit || 10000) > 0.8 ? 'bg-red-500' : 'bg-blue-500'}`} 
                  style={{ width: `${Math.min(((apiQuota?.youtubeUsed || 0) / (apiQuota?.youtubeLimit || 10000)) * 100, 100)}%` }} 
                />
              </div>
            </div>
